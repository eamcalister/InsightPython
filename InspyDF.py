...
if __name__ == "__main__":
    ...
    insightmaker_model_path = 'SomeoneElseFoxRabbit.InsightMaker'
...
#V2: To compare with varying IC. You need to look up where the ICs were and change them to X1 manually.

#X1 = np.array([0.01,.41])

#if len(model_parameters)>0:
#    model = lambdify((t, stocks) + tuple(parameter_symbols), stocks_dot)
#    solution2 = scipy.integrate.solve_ivp(model, (simulation_settings[0], simulation_settings[0]+simulation_settings[1]), X1,method=SolMethod, t_eval = tspan, args = args)
#else: 
#    model = lambdify((t, stocks), stocks_dot)
#    solution2 = scipy.integrate.solve_ivp(model, (simulation_settings[0], simulation_settings[0]+simulation_settings[1]), X1,method=SolMethod, t_eval = tspan)
...
# Plot the solutions for both variables
plt.plot(tspan, solution.y[0])
#plt.plot(tspan, solution2.y[0])
fig, ax = plt.subplots()
ax.plot(solution.y[0], solution.y[1])
#ax.plot(solution2.y[0], solution2.y[1])

#plt.plot(t, X[:, 1], label='y1(t)')
#plt.xlabel('Time')
